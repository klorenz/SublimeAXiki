...
from .xiki import util, core, core, path, interface, parser
...
class SublimeXiki(BaseXiki):
    ...
    def is_vintageous(self, view):
        ai = view.settings().get('vintageous_autoindent')
        ip = view.settings().get('ignored_packages')
        if 'Vintageous' in ip:
            return False
        if ai is None:
            return False
    ...
    def is_vintage(self, view):
        ip = view.settings().get('ignored_packages')
        return 'Vintage' not in ip
    ...
    def makedirs(self, path):
        if self.is_packages(path):
            _path = sublime.packages_path()
            path = path[9:]
            dir = os.path.join(_path, path.split('/')[0])
            if not os.path.exists(dir):
                raise OSError(13, "Permission denied: %s", repr(path))
            path = os.path.join(_path, path)

        return BaseXiki.makedirs(self, path)
    ...
    def __call__(self, path=None, input=None, action="default", view=None, cont=False, handle_input=False, data=None, edit=None):
        ...
        '''
        if path:
            xiki = SublimeRequestXiki(self, view, line_region=None, tree=path, cont=cont, sel=sel)

            if action in ('open', 'default'):
                if not input: input = ""
                output = xiki.open(input=input, cont=cont)

            elif action == 'close':
                if not input: input = ""
                output = xiki.close(input=input)
            ...
            if not edit:
                with Edit(view) as edit:
                    edit.replace(view.sel()[0], output)
            else:
                view.replace(edit, view.sel()[0], output)
...
class SublimeRequestXiki(SublimeXiki, ProxyXiki):
    ...
    def expand_dir(self, name):
        ...
        return BaseXiki.expand_dir(self, name)
    ...
    def open_file(self, filename, opener=None, text_opener=None, bin_opener=None, content=None):
        ...
        def text_opener(filename):
            ...
            view.settings().set('xiki_activate_on_close', self.view.id())
...
class XikiHandlerThread(threading.Thread):
    ...
    def _insert_snippet(self, contents):
        ...
        if view.settings().get('command_mode'):
            if self.xiki.is_vintageous(view):
                view.run_command('set_action', {"action": "vi_enter_insert_mode"})
...
class XikiListener(sublime_plugin.EventListener):
    ...
    def on_query_completions(self, view, prefix, locations):
        ...
        return xiki(view=view, action='complete', data={
            'before': view.substr(sublime.Region(loc-100, loc)),
            'prefix': prefix,
            'after' : view.substr(sublime.Region(loc, loc+100)),
            }
            )
...
class XikiCommand(sublime_plugin.TextCommand):
    ...
    def run(self, edit, path=None, input=None):
        xiki(view=self.view, path=path, input=None, edit=edit)
...
class XikiContinue(XikiCommand):
    def run(self, edit):
        xiki(view=self.view, cont=True)

class XikiInputCommand(XikiCommand):
    def run(self, edit):
        xiki(view=self.view, handle_input=True)
...
def plugin_loaded():
    # make xiki available in main environment
...
#   xiki.register_plugin('User/aXiki')
    xiki.extensions()
